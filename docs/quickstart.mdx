...
generate:
  postgres:
  - modelName: "Todo"
    modelPackage: "github.com/example/todo/gnorm/dbl"
    modelPackageShort: "dbl"
    postgresName: "Todo"
    primaryKey: "TodoID"
    primaryKeyType: "int"
  - modelName: "User"
    modelPackage: "github.com/example/todo/models"
    modelPackageShort: "models"
    postgresName: "User"
    primaryKey: "UserID"
    primaryKeyType: "int"
```
...
gqlgen allows us to define our own struct to use for Todo instead of the automatically generated one, and that's what we'll do here.  Sometimes you may want to create your own, but in this case we'll just use the automatically generated database struct for Todo.  Update `gqlgen.yml` at the bottom with the following to tell gqlgen that we are going to provide the Todo model ourselves:

```
models:
  Todo:
    model: github.com/example/todo/gnorm/dbl.Todo
```

Because we're separating the database representation of our data from the GraphQL API, we need to perform the translation between the database's model and our GraphQL model.  In this case, the translation is very simple because we told gqlgen to use our database model directly.  However, you may sometimes want to override this and perform your own translation.  Create the file `loader/todo.go` with the following content:
...
func hydrateModelTodo(ctx context.Context, i dbl.TodoFull) (dbl.Todo) {
	return i.Todo
}
```

Because we're using the auto-generated model for User, we need to do some manual translation ourselves.  We could do the same as above, overriding the model with our own, but this demonstrates how it would look if we manually hydrated the models/User struct.  In `loader/user.go` we'll create the following:
...
In `loader/init.go`, update the runBatchLoaders() function with the ones for our new functions.  This lets us batch together some requests to reduce database calls:
...
func (r *mutationResolver) CreateTodo(ctx context.Context, input models.NewTodo) (*dbl.Todo, error) {
...
func (t *todoResolver) User(ctx context.Context, obj *dbl.Todo) (*models.User, error) {
	user, err := loader.Loader.GetUser(ctx, obj.UserID)
	return &user, err
}

func (t *todoResolver) ID(ctx context.Context, obj *dbl.Todo) (string, error) {
